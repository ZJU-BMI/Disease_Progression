...
def save_result(return_data_map, file_path, name_prefix):
    ...
    fm_decay_function = name_prefix + 'decay_function.csv'
    fm_decay_integral = name_prefix + 'decay_function_integral.csv'
    ...
    if return_data_map['kernel'] == 'Fourier' or return_data_map['kernel'] == 'fourier':
        ...
        with open(os.path.join(file_path, fm_decay_function), 'w', encoding='utf-8-sig', newline="") as f:
            csv_writer = csv.writer(f)
            decay_function = return_data_map['time_decay_function']
            csv_writer.writerows([decay_function])

        with open(os.path.join(file_path, fm_decay_integral), 'w', encoding='utf-8-sig', newline="") as f:
            csv_writer = csv.writer(f)
            decay_integral = return_data_map['time_decay_integral']
            csv_writer.writerows([decay_integral])
...
def hawkes_optimization(train_data, test_data, iteration, diagnosis_reserve, procedure_reserve, kernel,
    ...
    if kernel == 'fourier' or kernel == 'Fourier':
        ...
        return_data_map['time_decay_function'] = hawkes_process.discrete_time_decay
        return_data_map['time_decay_integral'] = hawkes_process.discrete_time_integral
...
def hawkes_eval():
    ...
    # source data server: os.path.abspath('/mnt/datashare/group.huang/diseaseprogression/reconstruct_data/mimic_3
    # /reconstruct/')
    # source data local: os.path.abspath('..\\..\\..') + '\\reconstruct_data\\mimic_3\\reconstruct\\'
    # save data server: os.path.abspath('/mnt/datashare/group.huang/diseaseprogression/reconstruct_data/mimic_3
    # /reconstruct/')
