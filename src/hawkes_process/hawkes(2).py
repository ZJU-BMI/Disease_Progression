...
class HawkesOrigin(object):
    ...
    def __init__(self, training_data, test_data, event_count, hyper_parameter, kernel, init_strategy, time_slot):
        ...
        if self.excite_kernel == 'fourier' or self.excite_kernel == 'Fourier':
            self.count_each_slot_list = self.event_number_of_each_slot()
            self.event_count_list = self.event_number_of_each_event()
    ...
    def event_number_of_each_event(self):
    ...
    def event_number_of_each_slot(self):
        ...
        time_unit = (event_list_full[-1][1] - event_list_full[0][1]) // self.time_slot + 1
        ...
        for j in self.training_data:
            ...
            for i in range(0, list_length):
                ...
                for k in range(0, list_length):
                    variable_sum += self.auxiliary_variable[j][i][k]
                if variable_sum > 1.001 or variable_sum < 0.999:
                    raise Exception('sum not equal 1')
    ...
    def kernel_calculate(self, early_event_time, late_event_time):
        ...
        if kernel_type == 'default' or kernel_type == 'exp':
            ...
            for k in range(0, self.time_slot + 1):
                ...
                y_omega_k = 0
                for i in range(0, self.time_slot):
                    y_omega_k += cmath.exp(-1 * omega_k * i * complex(0, 1)) * self.count_each_slot_list[i]
...
class Hawkes(HawkesOrigin):
    ...
    def k_omega_update(self):
        ...
        for k in range(0, self.time_slot):
            if k == 0:
                k_denominator[k][0] = np.dot(self.mutual_intensity, self.event_count_list).sum()
            else:
    ...
    def output_result(self, file_path):
        ...
        if self.excite_kernel == 'Fourier' or self.excite_kernel == 'fourier':
            ...
            with open(file_path + event_number_each_slot, 'w', encoding='utf-8-sig', newline="") as f:
                csv_writer = csv.writer(f)
                csv_writer.writerows([self.count_each_slot_list])
...
def main():
    ...
    for diagnosis_no in [10, 20]:
        for operation_no in [0]:
            ...
            index_map_name = 'index_name_map_diagnosis_'+str(diagnosis_no)+'_operation_'+str(operation_no)+'.csv'
